...
def add_sdf_collider(
    ...
    def sdf_collider(
        model: MPMModel, x: Tensor, v: Tensor, bound: float, dim: int, start_time: float, end_time: float
    ):
        time = model.time
...
def enforce_particle_velocity_translation(
    model: MPMModel, point: list, size: list, velocity: list, start_time: float, end_time: float
):
    point = torch.tensor(point, device=model.device).float()
    size = torch.tensor(size, device=model.device).float()
    velocity = torch.tensor(velocity, device=model.device).float()
    ...
    def enforce_velocity_translation(
        model: MPMModel, x: Tensor, v: Tensor, target: Tensor, velocity: Tensor, start_time: float, end_time: float
    ):
        time = model.time
        if time >= start_time and time < end_time:
            v[target] = velocity
...
def enforce_particle_velocity_rotation(
    ...
    target = torch.all(
        (vertical_distance < half_height_and_radius[0]) & (horizontal_distance < half_height_and_radius[1]), dim=1
    )
...
def add_surface_collider(
    ...
    def collide(model: MPMModel, target: Tensor, surface: str, start_time: float, end_time: float):
        time = model.time
        if time >= start_time and time < end_time:
            ...
            # fix it
                ...
                model.grid_mv[target] = model.grid_mv[target] - normal * torch.sum(
                    model.grid_mv[target] * normal, dim=1, keepdim=True
                )
            elif surface == "collide":
                # set the vertical velocity to be opposite
                model.grid_mv[target] = model.grid_mv[target] - normal * 2.0 * torch.sum(
                    model.grid_mv[target] * normal, dim=1, keepdim=True
                )
            else:
