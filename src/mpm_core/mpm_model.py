...
class MPMModel:
    def __init__(
        ...
        self.grid_x = (
            torch.stack((grid_x, grid_y, grid_z), dim=-1).reshape(-1, 3).float()
        )  # (n_grid * n_grid * n_grid, 3)

        self.offset = torch.tensor(
            [[i, j, k] for i in range(3) for j in range(3) for k in range(3)], device=device
        ).float()  # (27, 3)
    ...
    def p2g2p(
        ...
        dweight = inv_dx * torch.stack(dweight, dim=-1).reshape(
            -1, 27, 3
        )  # (n_particles, 3, 3, 3, 3) -> (n_particles, 27, 3)

        dpos = (self.offset - fx.unsqueeze(1)) * dx  # (n_particles, 27, 3)

        index = base.unsqueeze(1) + self.offset.unsqueeze(0).long()  # (n_particles, 27, 3)
        index = (index[:, :, 0] * n_grids * n_grids + index[:, :, 1] * n_grids + index[:, :, 2]).reshape(
            -1
        )  # (n_particles * 27)
        index = index.clamp(
            0, n_grids**3 - 1
        )  # (n_particles * 27) TODO: simple clipping leads to some numerical problems, but it's acceptable for now.
        ...
        # p2g
        mv = -dt * vol * torch.einsum("bij, bkj -> bki", stress, dweight) + p_mass * weight.unsqueeze(2) * (
            v.unsqueeze(1) + torch.einsum("bij, bkj -> bki", C, dpos)
        )  # (n_particles, 3, 3), (n_particles, 27, 3) -> (n_particles, 27, 3)
        ...
        C = torch.einsum(
            "bij, bik -> bijk", v, dpos
        )  # (n_particles, 27, 3), (n_particles, 27, 3) -> (n_particles, 27, 3, 3)
        new_F = torch.einsum(
            "bij, bik -> bijk", v, dweight
        )  # (n_particles, 27, 3), (n_particles, 27, 3) -> (n_particles, 27, 3, 3)
